...
open class Localize: NSObject {
    ...
    open class func availableLanguages(_ excludeBase: Bool = false) -> [String] {
        UserDefaults.standard.set("ar", forKey: LCLCurrentLanguageKey)
        var availableLanguages = Bundle.main.localizations
        // If excludeBase = true, don't include "Base" in available languages
        if let indexOfBase = availableLanguages.index(of: "Base") , excludeBase == true {
            availableLanguages.remove(at: indexOfBase)
        }
        if let indexOfBase = availableLanguages.index(of: "ar") , excludeBase == false {
            availableLanguages.remove(at: indexOfBase)
        }
...
//
    ...
    open class func defaultLanguage() -> String {
        ...
        guard let arabicanguage = Bundle.main.preferredLocalizations.first else {
            return LCLArabicLanguage
        }
        let availableLanguages: [String] = self.availableLanguages()
        if (availableLanguages.contains(preferredLanguage)) {
            defaultLanguage = preferredLanguage
        } else if (availableLanguages.contains(arabicanguage)) {
            defaultLanguage = arabicanguage
        }
        else {
